...
TH1F* hist_prob3_d = new TH1F("step 4", "Histogram", 100,0., 300);
...
hist_prob3_d->GetXaxis()->SetTitle("Invariant mass, Gev");
...
float tau_eta[50];
float tau_phi[50];
bool tau_idAntiEleTight[50];
bool tau_idAntiMuTight[50];
...
Tree->SetBranchAddress("Tau_idAntiEleTight", tau_idAntiEleTight);
Tree->SetBranchAddress("Tau_idAntiMuTight", tau_idAntiMuTight);
...
for (int i = 0; i < int(nentires); i++){
    ...
    if (tau_charge[0]*tau_charge[1]<0 && absolute(tau_eta[0]) < 2.3 && tau_pt[0]  > 20 && tau_idDecayMode[0] == true && tau_idIsoTight[0] == true && tau_idDecayMode[1] == true && tau_idIsoTight[1] == true && tau_idAntiEleTight[0] == false && tau_idAntiMuTight[0] == false && tau_idAntiEleTight[1] == false && tau_idAntiMuTight[1] == false){
